...
姓名、签名、标题和“与 `[主题人物名]` 一起，用灵感创造无限可能”等文案不能烘焙进背景位图。首页主标题应写入跨平台 `homeTitle` 字段；主题名用于两端的切换入口。其他未形成可见契约的文案仍需先实现对应 schema 与真实 UI 层，再由 UI 渲染；不要仅写入未知的 `theme.json` 字段就假定会显示。
