...
1. 在 GitHub 的 [Releases](https://github.com/YuxiangChai/Codex-Skin/releases) 下载最新的
...
5. 应用启动后会在用户目录部署或升级主题引擎。菜单中的“安装 App 内置引擎”会部署缺失的引擎；
   “验证 / 修复本机引擎”会在确认后自动重启 ChatGPT、重新安装 App 内的同版本引擎并重新应用皮肤。
...
从 Developer ID 签名并公证的版本开始，菜单栏中的“检查更新…”会在发现新版本后显示确认框。
选择“自动更新”后，客户端会从固定的个人 GitHub Release 下载 DMG，核对文件名、URL、大小和
SHA-256，再校验 App 的 Bundle ID、版本、内置引擎、Apple Team ID 与 Gatekeeper 结果。随后它会
...
Gatekeeper。每个新下载的、未签名应用副本仍可能再次触发系统检查；若出现提示，重复上面的
“隐私与安全性 → 仍要打开”即可，不需要重新安装引擎。

菜单栏会每 24 小时在后台静默访问一次 GitHub Releases（启动后约 15 秒先查一次），
发现新版本时推送一条系统通知并在菜单里显示「🆕 发现新版本」；「维护」子菜单里的
「立即检查更新」可随时手动触发。后台检查只通知，不会静默下载或替换应用；用户在手动检查或
通知入口中确认后，个人版本才会下载、校验并执行带备份和回滚的自动更新。
