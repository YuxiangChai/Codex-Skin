...
当前个人发行使用 ad-hoc 签名，不会要求关闭 Gatekeeper 或执行 `xattr`。首次安装，或 macOS 实际
重新审查某个替换版本时，系统仍可能要求确认一次；已经允许过的应用通常可以直接完成应用内更新。
...
- 更新完成后会自动重启 Codex 并恢复当前皮肤；正常情况下无需打开“隐私与安全性”。只有新版 App
  在受限时间内始终没有确认启动时，才会把系统设置作为 Gatekeeper 兜底提示。
...
- 暂停主题或只停止 injector 不会关闭已经启动的调试端口；完整恢复并重启，或退出全部 Codex 后从官方入口重新打开，风险窗口才会结束。
- 不修改官方应用安装目录、二进制文件或代码签名。
- 不会自动改写 API Key、模型供应商或 Base URL；中转与换肤彼此独立。
