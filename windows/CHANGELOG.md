...
- 同步 Codex 26.810.41047 `from-surface` session footer 渐变兼容与共享回归测试，保留旧版 token 类名。
...
- 同步用户消息主题表面修复：主题色只作用于原生圆角气泡，外层消息行保持透明；旧版 role 结构继续使用兼容回退。

### 个人集成 1.5.11.1

- 同步合并上游 v1.5.10 / v1.5.11 的社区主题恢复、Safe CSS、通用 renderer 和 Codex 26.727 Settings 兼容，同时保留个人主题合同；Windows PowerShell/安装器仍需 CI 或 Windows 实机验证。

### 修复

- 同步 macOS 的 ChatGPT Desktop 26.727.40816 renderer 选择器兼容合同：保留旧语义类，同时接受稳定 CSS Modules 模块名前缀且不匹配随机 hash。Windows 本机 26.727 DOM 尚待 CI/实机复核，辅助 renderer 的严格拒绝条件不变。
### 新增
