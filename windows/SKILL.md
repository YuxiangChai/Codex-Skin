...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the Iron Man theme, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout.
...
- `assets/dream-reference.jpg`: redistributable AI-generated Iron Man wallpaper seeded as the default and as the sole bundled saved theme; it contains no Codex UI.
