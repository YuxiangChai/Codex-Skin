...
"$NODE" -e '
  ...
  if (value.currentVersion !== "v1.5.16.2" || value.latestVersion !== "v9.8.7") process.exit(1);
...
"$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (value.currentVersion !== "v1.5.16.2" || value.latestVersion !== "v1.5.16") process.exit(1);
  if (value.updateAvailable) process.exit(1);
' "$STABLE_AFTER_PERSONAL_JSON"
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.5.16.2" ]' _ "$ROOT"
