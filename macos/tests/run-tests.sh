...
"$NODE" -e '
  ...
  if (value.currentVersion !== "v1.5.14.1" || value.latestVersion !== "v9.8.7") process.exit(1);
...
STABLE_AFTER_PERSONAL_JSON="$({
  CODEX_DREAM_SKIN_TEST_RESPONSE_FILE="$ROOT/tests/fixtures/latest-release-stable-after-personal.json" \
    "$ROOT/scripts/check-update-macos.sh" --json
})"
"$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (value.currentVersion !== "v1.5.14.1" || value.latestVersion !== "v1.5.15") process.exit(1);
  if (!value.updateAvailable) process.exit(1);
' "$STABLE_AFTER_PERSONAL_JSON"
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.5.14.1" ]' _ "$ROOT"
