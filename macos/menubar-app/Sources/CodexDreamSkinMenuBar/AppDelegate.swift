...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, UNUserNotificationCenterDelegate {
  ...
  private let requiredEngineRelativePaths = [
    ...
    "presets/preset-iron-man/theme.css",
    "presets/preset-iron-man/theme.json",
    ...
    ScriptRunner.run(script: script, arguments: ["--json"]) { [weak self] result in
      ...
      guard result.succeeded,
        ...
        self.offerUpdateRetry(
          ...
          buttonTitle: self.copy.resolvedLanguage == .chinese ? "重新检查" : "Check again"
        ) { [weak self] in
      ...
      if available {
        ...
        alert.informativeText = self.copy.resolvedLanguage == .chinese
          ...
          : "Current version: \(current). The app will first download and verify the \(ByteCountFormatter.string(fromByteCount: assetBytes, countStyle: .file)) installer, then ask before installing. The new app will reopen Codex and restore the active skin; macOS approval is needed only if launch is blocked. Images and saved themes are preserved."
        alert.addButton(withTitle: self.copy.resolvedLanguage == .chinese ? "下载更新" : "Download update")
  ...
  private func performBackgroundUpdateCheck() {
    ...
    ScriptRunner.run(script: script, arguments: ["--json"]) { [weak self] result in
      ...
      let releaseURL = (value["releaseUrl"] as? String)
        ?? "https://github.com/YuxiangChai/Codex-Skin/releases/latest"
  ...
  private func postUpdateAvailableNotification(version: String, releaseURL: String) {
    ...
    if copy.resolvedLanguage == .chinese {
      ...
      content.body = "\(version) 已发布，点按下载、校验并安装更新。"
    } else {
      content.title = "Codex Dream Skin update available"
      content.body = "\(version) is available. Click to download, verify, and install it."
    }
  ...
  private func installBundledEngineIfNeeded(force: Bool) {
    ...
    ScriptRunner.run(
      ...
      if result.succeeded {
        ...
        self.showError(
          title: self.pendingSelfUpdate == nil
            ? self.copy.text(.installFailedTitle)
            : (self.copy.resolvedLanguage == .chinese ? "更新后的引擎安装未完成" : "Updated engine installation did not finish"),
          message: self.conciseOutput(
            result.output,
            fallback: self.pendingSelfUpdate == nil
              ? self.copy.text(.installFailedMessage)
              : (self.copy.resolvedLanguage == .chinese
                  ? "新版 App 尚未确认更新完成，旧 App 备份仍然保留。请重试引擎安装。"
                  : "The new app has not confirmed the update. The old app backup is still preserved; retry the engine installation.")
          )
        )
      }
    }
