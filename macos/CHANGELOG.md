...
- API 403 不再被无意义地连续重试，也不会把四次错误直接显示给用户；后台检查和手动检查共用同一套有界元数据与下载后二次校验链路。

### 个人发行 1.5.16.2

- 修复 Codex 26.825 在 Dream Skin 启动时序下可能永久卡住长文本附件初始化的问题：继续使用官方 `Pasted text` 小附件卡片，不把长文本直接塞入输入框；只对唯一、超时且结构匹配的官方注册表读取做一次有界恢复。
- 修复 Session 中 Agent 回复完成时左上角可能快速闪白的问题，保留思考玻璃表面，但避免完成瞬间销毁模糊合成层造成的闪烁。
- 本次仍只构建和发布 macOS DMG；Windows 仅同步版本与共享运行时，不构建或上传 Setup.exe。
...
- 合并上游 v1.5.12 至 v1.5.14：加入 macOS 双语菜单、后台更新通知、渲染可见性加固、图片尺寸预检和主题 schema 修复，并同步 Windows 冷启动与失败恢复安全改进。
- 保留个人自动下载、校验、原子替换、重启 Codex 和恢复当前皮肤的更新流程，以及 Iron Man 单主题、主区域图片焦点、统一首页标题与紧凑输入框。
- 适配 Codex 26.810.41047 新的 `from-surface` session footer 渐变类，清除输入框下方重新出现的原生阴影带，同时保留旧类名兼容。
...
- 用户确认安装后自动退出 Codex、停止旧注入器并关闭 Dream Skin；安装助手自动挂载和卸载 DMG、保留原子备份与失败回滚，并在新版 App 安装匹配引擎后恢复 Codex 和当前皮肤。
- 修复 App 已更新、但旧 Codex 仍在运行，导致新版启动时引擎安装被 `config.toml` 写入保护拒绝的问题。更新确认现在延后到匹配引擎成功安装，失败时保留旧 App 备份和可重试状态。

### 个人发行 1.5.11.2

- 修复用户消息主题色错误地作用于整条消息行、在原生圆角气泡后形成全宽长方形色块的问题。新版只给 `[data-user-message-bubble]` 原生圆角表面着色，外层消息行保持透明，并保留旧版 role 结构的兼容回退。

### 个人集成 1.5.11.1

- 合并上游 v1.5.10 / v1.5.11：增加社区主题原子升级与中断恢复、Safe CSS 合同改进、Codex 26.727 通用 renderer 与新版 Settings 面板识别。
- 保留 Iron Man 单主题、主题自定义首页文案与布局、主对话区域居中的共享背景、主题暗度、用户消息主题色和 pinned summary 默认关闭。
- 个人集成版本从已验证的合并提交构建；发布安装包不会自动替换用户当前安装的引擎。
...
- Pinned summary 默认保持关闭，只有用户亲自点击后才保持展开；切换对话后重新恢复默认关闭。
- Iron Man 背景暗度调整为 `0.5`，Settings 复用同一张稳定背景并消除首次进入时的短暂不透明闪烁。
- 保留 Chat、Work、Codex 统一的 Jarvis 首页标题、底部紧凑输入框、主对话区域居中的共享背景和透明边框。

### 个人发行 1.5.9.1

- 增加 ChatGPT 26.727 renderer 选择器兼容，保留 Jarvis 首页、底部紧凑输入框与主区域居中的共享 Iron Man 背景。
- 增加验证式引擎修复，以及带备份、启动确认和回滚的一键更新。
- ad-hoc 包支持 Gatekeeper 辅助更新：自动下载、校验和替换后打开“隐私与安全性”，用户只需点击一次“仍要打开”。
- Iron Man 主题的主要界面边框改为透明，并继续由主题控制暗度和视觉令牌。
- 个人发行版本允许 `上游三段版本.个人修订号`，例如 `1.5.9.1`。
...
- 将未发布的本地 v1.5.10 工作整理为个人发行 v1.5.9.1，避免占用后续上游 v1.5.10 版本号。
